    ...
    , ui(new Ui::StartScene)
{
    ...
    connect(signinBtn1,&MyPushButton::clicked,[=](){
        chooseModeScene = new ChooseModeScene;  //实例化登录后模式界面
        QTimer::singleShot(200,this,[=](){  //延时0.2秒
            this->close();  //自身关闭
            chooseModeScene->show();  //登录后模式界面
        });
    });
    ...
    //退出游戏
    connect(signinBtn4,&QPushButton::clicked,this,&StartScene::close);

}

void StartScene::paintEvent(QPaintEvent *)  //重写paintEvent事件，画背景图
{
